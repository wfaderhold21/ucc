...
Every context is owned by its library until context destruction succeeds.
Every posted team and every successfully mapped-memory handle is owned by its
context until team destruction or memory unmapping succeeds. Applications must
destroy teams and unmap memory before destroying their context, and destroy all
contexts before finalizing their library. An out-of-order destroy/finalize
returns `UCC_ERR_INVALID_PARAM` before teardown; the refused parent remains
usable and cleanup may be retried. This rule is unconditional and has no

                                         ...
                                         ucc_schedule_pipelined_t *schedule)
{
    ...
    if (ucc_unlikely(n_frags < 1)) {
        ucc_error("n_frags=%d is invalid for active pipeline; must be >= 1",
                  n_frags);
        return UCC_ERR_INVALID_PARAM;
    }

    if (ucc_unlikely(n_frags_total < n_frags || n_frags_total <= 0)) {
        ucc_error("n_frags_total=%d is invalid (n_frags=%d); must be >= n_frags and > 0",
                  n_frags_total, n_frags);
        return UCC_ERR_INVALID_PARAM;
    }

    if (ucc_unlikely(order != UCC_PIPELINE_PARALLEL &&
                     order != UCC_PIPELINE_ORDERED &&
    ...
    if (ucc_unlikely(n_frags > UCC_SCHEDULE_PIPELINED_MAX_FRAGS)) {
        ucc_warn("n_frags %d exceeds max limit of %d, clamping",

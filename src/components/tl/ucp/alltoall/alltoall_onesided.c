...
ucc_status_t ucc_tl_ucp_alltoall_onesided_sm_progress(ucc_coll_task_t *ctask);
...
/*            if (grank == 2) {
    ...
    peer = start;
    for (peer = start; task->send_posted < gsize; peer = (peer + 1) % gsize) {
        if ((task->send_posted - task->send_completed) < nreqs) {
...
ucc_status_t ucc_tl_ucp_alltoall_onesided_sm_start(ucc_coll_task_t *ctask)
{
...
#if 0
    while (*pSync < stride || task->send_completed < task->send_posted) {
        ucp_worker_progress(UCC_TL_UCP_TEAM_CTX(team)->ucp_worker);
    }   
    ...
    if (nr_nodes > 1) {     
        ...
        if (grank == node_leader) {
            ...
            while (*pSync < (stride + nr_nodes) || task->send_completed < task->send_posted) {
                ucp_worker_progress(UCC_TL_UCP_TEAM_CTX(team)->ucp_worker);
            }
...
/*    if (nelems < 131072) { 
    ...
    dest   = dest + grank * nelems;
    for (peer = start; task->send_posted < gsize;) {

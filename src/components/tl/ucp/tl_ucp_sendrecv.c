...
static ucc_rank_t
ucc_tl_ucp_sender_tag_to_rank(const ucp_tag_recv_info_t *info)
{
    return (ucc_rank_t)
        ((info->sender_tag >> UCC_TL_UCP_SENDER_BITS_OFFSET) &
...
static inline void ucc_tl_ucp_handle_send_status(ucc_tl_ucp_task_t *task,
                                                 ucs_status_t status,
                                                 ucc_rank_t rank)
{
    ucc_status_t ucc_st;
                                      ...
                                      void *user_data)
{
    ...
    ucc_tl_ucp_handle_send_status(task, status, UCC_RANK_MAX);
...
void ucc_tl_ucp_send_completion_ranked_cb_st(void *request, ucs_status_t status,
                                             void *user_data)
{
    ucc_tl_ucp_send_completion_arg_t *arg = user_data;
    ucc_tl_ucp_task_t                *task = arg->task;

    ucc_tl_ucp_handle_send_status(task, status, arg->rank);
    ++task->tagged.send_completed;
    ucp_request_free(request);
    ucc_free(arg);
}

void ucc_tl_ucp_send_completion_ranked_cb_mt(void *request, ucs_status_t status,
                                             void *user_data)
{
    ucc_tl_ucp_send_completion_arg_t *arg = user_data;
    ucc_tl_ucp_task_t                *task = arg->task;
...
void ucc_tl_ucp_send_completion_wrapped_cb(void *request, ucs_status_t status,
                                           void *user_data)
{
    ucc_tl_ucp_send_completion_arg_t *arg = user_data;

    ucc_tl_ucp_handle_send_status(arg->task, status, arg->rank);
    arg->cb(request, status, arg->user_data);
    ucc_free(arg);
}

static inline void
ucc_tl_ucp_handle_recv_status(ucc_tl_ucp_task_t *task, ucs_status_t status,
                              ...
                              ucc_rank_t rank)
{
    ...
    task->super.status = ucc_st;
    if (ucc_st == UCC_ERR_COMM_FAILURE) {
        ucc_tl_ucp_mark_peer_failed(TASK_TEAM(task),
                                    info ? ucc_tl_ucp_sender_tag_to_rank(info)
                                         : rank);
    }
}

void ucc_tl_ucp_recv_completion_wrapped_cb(void *request, ucs_status_t status,
                                           const ucp_tag_recv_info_t *info,
                                           void *user_data)
{
    ucc_tl_ucp_recv_completion_arg_t *arg = user_data;

    ucc_tl_ucp_handle_recv_status(arg->task, status, info, arg->rank);
    arg->cb(request, status, info, arg->user_data);
    ucc_free(arg);
}
                                      ...
                                      void *user_data)
{
    ucc_tl_ucp_task_t *task = (ucc_tl_ucp_task_t *)user_data;
    if (ucc_unlikely(UCS_OK != status)) {
        ucc_tl_ucp_handle_recv_status(task, status, info, UCC_RANK_MAX);
    }
                                 ...
                                 ucc_tl_ucp_task_t *task)
{
    ...
    ucp_request_param_t                 ranked_req_param = *req_param;
    ucc_status_t                        status;
    ucp_ep_h                            ep;
    ucp_tag_t                           ucp_tag;
    ...
    status = ucc_tl_ucp_prepare_send_completion_arg(
        team, dest_group_rank, task, &ranked_req_param.cb.send,
        &ranked_req_param.user_data, &cb_arg);
    if (ucc_unlikely(status != UCC_OK)) {
        return status;
    }

    ucp_status = ucp_tag_send_nbx(ep, buffer, msglen, ucp_tag,
                                  &ranked_req_param);

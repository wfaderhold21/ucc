...
ucc_status_t ucc_cl_urom_triggered_post(ucc_ee_h ee, ucc_ev_t *ev,
...
static ucc_status_t ucc_cl_urom_allreduce_full_start(ucc_coll_task_t *task)
{
    ...
    if (coll_args->src.info.mem_type == UCC_MEMORY_TYPE_CUDA) {
        ...
        if (ctx->req_mc) {
            size_t size_mod = dt_size(coll_args->src.info.datatype);
            size_t count = coll_args->src.info.count * size_mod;
            ...
            ucc_mc_memcpy(ptr, ctx->old_src /*coll_args->args.src.info.buffer*/, count, UCC_MEMORY_TYPE_HOST, UCC_MEMORY_TYPE_CUDA);
        }
                         ...
                         ucc_coll_task_t **task)
{
    ...
    schedule->super.triggered_post = ucc_cl_urom_triggered_post;

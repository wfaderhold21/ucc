                                     ...
                                     ucc_ee_type_t *ee_type)
{
    ...
    if (!plugin_desc->ops.create_event || !plugin_desc->ops.destroy_event) {
        ucc_error("plugin must provide create_event and destroy_event operations");
        return UCC_ERR_INVALID_PARAM;
    }
    ...
    /* Allocate new entry */
    entry = (ucc_ec_plugin_entry_t*)ucc_malloc(sizeof(ucc_ec_plugin_entry_t),
                                                "ec_plugin_entry");
    if (!entry) {
    ...
    if (plugin_desc->version) {
        entry->desc.version = ucc_strdup(plugin_desc->version, "ec_plugin_version");
    }
    ...
    *ee_type = et;
    ...
    ucc_info("EC plugin '%s' registered with ee_type=%d",
             plugin_desc->name, et);
...
ucc_status_t ucc_ec_plugin_unregister(ucc_ee_type_t ee_type)
{
    ...
    if (ee_type < UCC_EE_LAST) {
        ucc_error("cannot unregister builtin execution engine type %d", ee_type);
        return UCC_ERR_INVALID_PARAM;
    }
    ...
    while (entry) {
        if (entry->ee_type == ee_type) {
            if (entry->ref_cnt > 0) {
                ucc_warn("EC plugin ee_type=%d still has %d references",
                        ee_type, entry->ref_cnt);
                pthread_mutex_unlock(&ec_plugin_mutex);
...
ucc_status_t ucc_ec_plugin_query(ucc_ee_type_t ee_type,
                                  const ucc_ec_plugin_desc_t **plugin_desc)
{
    ucc_ec_plugin_entry_t *entry;

...
static ucc_status_t create_test_team(ucc_base_team_t **team_h, int thread_mode)
{
    ucc_context_t      *ctx;
...
static int              g_fail_after_frag_idx;  /* fail at this fragment init index (-1 = never) */

/* Finalize wrapper that increments the global counter. */
static ucc_status_t count_finalize(ucc_coll_task_t *task)
{
    g_total_finalizes++;
    return ucc_schedule_finalize(task);
}
    ...
    ucc_schedule_t **frag_p)
{
    ...
    /* Add one synthetic task per fragment. */
    {
        ...
        task->post     = [](ucc_coll_task_t *t) -> ucc_status_t { (void)t; return UCC_OK; };
        task->finalize = count_finalize;
        ucc_coll_task_construct(task);
        frag->tasks[frag->n_tasks++] = task;
    }

    frag->super.finalize = count_finalize;
    *frag_p               = frag;
    return UCC_OK;
}
...
UCC_TEST_F(test_schedule, pipelined_init_depth_16_success)
{
    ...
    status = create_test_team(&team, UCC_THREAD_SINGLE);
    ASSERT_EQ(UCC_OK, status) << "Failed to create test team";

    /* Request depth-16 pipeline (matches UCC_SCHEDULE_PIPELINED_MAX_FRAGS). */
    status = ucc_schedule_pipelined_init(&bargs, team, mock_frag_init,
                                         NULL, 16, 16,
                                         UCC_PIPELINE_PARALLEL, &sched);
...
UCC_TEST_F(test_schedule, pipelined_init_failure_at_zero)
{
    ...
    status = ucc_schedule_pipelined_init(&bargs, team, mock_frag_init,
                                         NULL, 8, 8,
                                         UCC_PIPELINE_PARALLEL, &sched);
    EXPECT_NE(UCC_OK, status) << "Init should fail when frag 0 is forced to error";
...
UCC_TEST_F(test_schedule, pipelined_init_failure_at_last)
{
    ...
    EXPECT_EQ(14, std::atomic_load(&g_total_finalizes))
        << "7 fragments with 1 task each => 7*2=14 finalizers (schedule + task)";
...
UCC_TEST_F(test_schedule, pipelined_nfrags_zero_rejected)
{
    ...
    status = ucc_schedule_pipelined_init(NULL, team, NULL, NULL, 0, 1,
                                         UCC_PIPELINE_PARALLEL, NULL);
...
UCC_TEST_F(test_schedule, pipelined_nfrags_total_lt_nfrags_rejected)
{
    ...
    status = ucc_schedule_pipelined_init(NULL, team, NULL, NULL, 5, 3,
                                         UCC_PIPELINE_PARALLEL, NULL);
    EXPECT_EQ(UCC_ERR_INVALID_PARAM, status)
...
UCC_TEST_F(test_schedule, pipelined_nfrags_total_zero_rejected)
{
    ...
    status = ucc_schedule_pipelined_init(NULL, team, NULL, NULL, 1, 0,
                                         UCC_PIPELINE_PARALLEL, NULL);
    EXPECT_EQ(UCC_ERR_INVALID_PARAM, status)
...
UCC_TEST_F(test_schedule, pipeline_params_pdepth_zero_rejected)
{
    ...
    status = ucc_pipeline_nfrags_pdepth(&params, 2048, &n_frags, &pipeline_depth);
    EXPECT_EQ(UCC_ERR_INVALID_PARAM, status)
...
UCC_TEST_F(test_schedule, pipeline_params_frag_size_zero_rejected)
{
    ...
    status = ucc_pipeline_nfrags_pdepth(&params, 2048, &n_frags, &pipeline_depth);
    EXPECT_EQ(UCC_ERR_INVALID_PARAM, status)
        << "frag_size=0 with msgsize>threshold must be rejected before division";
}
...
UCC_TEST_F(test_schedule, pipeline_params_exact_threshold_boundaries)
{
    ...
    ASSERT_EQ(UCC_OK, ucc_pipeline_nfrags_pdepth(
                          &params, 1023, &n_frags, &pipeline_depth));
    EXPECT_EQ(1, n_frags);
    EXPECT_EQ(1, pipeline_depth);

    ASSERT_EQ(UCC_OK, ucc_pipeline_nfrags_pdepth(
                          &params, 1024, &n_frags, &pipeline_depth));
    EXPECT_EQ(1, n_frags);
    EXPECT_EQ(1, pipeline_depth);

    ASSERT_EQ(UCC_OK, ucc_pipeline_nfrags_pdepth(
                          &params, 1025, &n_frags, &pipeline_depth));
    EXPECT_EQ(5, n_frags);
...
UCC_TEST_F(test_schedule, pipeline_params_zero_frag_size_threshold_boundary)
{
    ...
    n_frags = 77;
    pipeline_depth = 88;
    EXPECT_EQ(UCC_ERR_INVALID_PARAM, ucc_pipeline_nfrags_pdepth(
                                         &params, 1025, &n_frags,
                                         &pipeline_depth));
    EXPECT_EQ(77, n_frags) << "failure must not publish fragment count";
    EXPECT_EQ(88, pipeline_depth) << "failure must not publish usable depth";
...
UCC_TEST_F(test_schedule, pipeline_params_explicit_multifragment)
{
    ...
    ASSERT_EQ(UCC_OK, ucc_pipeline_nfrags_pdepth(
                          &params, 4096, &n_frags, &pipeline_depth));
    EXPECT_EQ(8, n_frags);
...
UCC_TEST_F(test_schedule, pipelined_negative_depth_rejected)
{
    EXPECT_EQ(UCC_ERR_INVALID_PARAM,
              ucc_schedule_pipelined_init(NULL, NULL, NULL, NULL, -1, 1,
                                          UCC_PIPELINE_PARALLEL, NULL));
}
...
UCC_TEST_F(test_schedule, pipelined_invalid_order_depth_one_rejected)
{
    EXPECT_EQ(UCC_ERR_INVALID_PARAM,
              ucc_schedule_pipelined_init(NULL, NULL, NULL, NULL, 1, 1,
                                          (ucc_pipeline_order_t)99, NULL));
}

UCC_TEST_F(test_schedule, pipelined_all_valid_orders_at_boundary)
{
    const ucc_pipeline_order_t orders[] = {UCC_PIPELINE_PARALLEL,
    ...
    for (auto order : orders) {
        ...
        ASSERT_EQ(UCC_OK, ucc_schedule_pipelined_init(
                              &bargs, team, mock_frag_init, NULL, 1, 1,
                              order, &sched));
...
UCC_TEST_F(test_schedule, pipelined_depth_above_limit_clamped)
{
    ...
    ASSERT_EQ(UCC_OK, create_test_team(&team, UCC_THREAD_SINGLE));
    ASSERT_EQ(UCC_OK, ucc_schedule_pipelined_init(
                          &bargs, team, mock_frag_init, NULL,
    ...
    ASSERT_NE(nullptr, sched.super.super.finalize);
    EXPECT_EQ(UCC_OK, sched.super.super.finalize(&sched.super.super));
    EXPECT_EQ(2 * UCC_SCHEDULE_PIPELINED_MAX_FRAGS,
              g_total_finalizes.load());
    destroy_test_team(team);
}
...
static ucc_status_t t473_subscription_fault(void)
{
    ...
    return attempt == t473_subscribe_fail_at.load() ? UCC_ERR_NO_MEMORY : UCC_OK;
}
...
static ucc_status_t t473_frag_finalize(ucc_coll_task_t *task)
{
    ...
    t473_live_listeners_at_finalize += t473_listener_count(task);
    t473_frag_finalizes++;
    ucc_status_t status = ucc_schedule_finalize(task);
    t473_pool_returns++;
    ucc_coll_task_destruct(task);
    free(frag);
    return status;
...
static ucc_status_t t473_frag_init(ucc_base_coll_args_t *args,
                                   ...
                                   ucc_schedule_t **frag_p)
{
    ...
    ucc_schedule_t *frag = (ucc_schedule_t *)calloc(1, sizeof(*frag));
    ucc_coll_task_t *task = (ucc_coll_task_t *)calloc(1, sizeof(*task));
    if (!frag || !task) {
        free(frag);
        free(task);
...
static void t473_expect_subscription_unwind(ucc_pipeline_order_t order,
                                            int fail_at, int thread_mode)
{
    ucc_base_team_t *team = NULL;
    ucc_schedule_pipelined_t sched;
    ...
    EXPECT_EQ(UCC_ERR_NO_MEMORY,
              ucc_schedule_pipelined_init(&args, team, t473_frag_init, NULL,
                                          6, 6, order, &sched));
    ...
    EXPECT_EQ(thread_mode == UCC_THREAD_MULTIPLE ? 1 : 0,
              t473_lock_inits.load());
...
{
    t473_expect_subscription_unwind(UCC_PIPELINE_SEQUENTIAL, 0,
                                    UCC_THREAD_SINGLE);
}
...
UCC_TEST_F(test_schedule, pipelined_real_subscription_failure_later)
{
    ...
    t473_expect_subscription_unwind(UCC_PIPELINE_ORDERED, 7,
                                    UCC_THREAD_MULTIPLE);
}
...
UCC_TEST_F(test_schedule, pipelined_depth_16_exact_lifecycle)
{
    ...
    t473_reset(-1, -1);
    ASSERT_EQ(UCC_OK, ucc_schedule_pipelined_init(
                          &args, team, t473_frag_init, NULL, 16, 16,
                          UCC_PIPELINE_PARALLEL, &sched));
...
UCC_TEST_F(test_schedule, pipelined_fragment_failure_exact_unwind)
{
    ...
    for (int fail_at : failures) {
        ...
        EXPECT_EQ(UCC_ERR_NO_MEMORY, ucc_schedule_pipelined_init(
                      &args, team, t473_frag_init, NULL, 8, 8,
                      UCC_PIPELINE_PARALLEL, &sched));
...
UCC_TEST_F(test_schedule, em_reuse_single_node)
{
    ...
    for (int i = 0; i < MAX_LISTENERS; i++) {
        EXPECT_EQ(UCC_OK, ucc_event_manager_subscribe(
                              &task, UCC_EVENT_COMPLETED, &listener_task,
...
UCC_TEST_F(test_schedule, em_nodes_freed_exactly_once)
{
    ...
    const int expected_em_nodes = (n_subscribers + MAX_LISTENERS - 1) / MAX_LISTENERS;
    ...
    EXPECT_EQ(expected_em_nodes, em_count)
        << "Expected " << expected_em_nodes << " EM nodes for "

...
public:
    ...
    ucc_status_t check() override {
        ...
        unsigned char expected = 0xAA + rank;
    ...
    void run(bool triggered) override {
        ...
        // Test memory map
        ucc_status_t status = ucc_mem_map(team.ctx, mode, &map_params, &memh_size, &memh);
        if (status != UCC_OK) {
            if (status == UCC_ERR_NOT_SUPPORTED || status == UCC_ERR_NOT_IMPLEMENTED) {
        ...
        if (!memh) {
            std::cerr << "Rank " << rank << ": Memory handle is NULL" << std::endl;
            return;
        }
        if (memh_size == 0) {
            std::cerr << "Rank " << rank << ": Memory handle size is 0" << std::endl;
            return;
        }
        ...
        if (memh != nullptr) {
            std::cerr << "Rank " << rank << ": Memory handle not NULL after unmap" << std::endl;
            return;
        }
...
public:
    ...
    static std::shared_ptr<TestCase> init_single(ucc_test_team_t &_team, ucc_coll_type_t _type, TestCaseParams params);
};
...
public:
    ...
    ucc_status_t check() override {
        ...
        // Verify buffer integrity
        unsigned char *buf = (unsigned char*)test_buffer;
        unsigned char expected = 0xBB + rank;
    ...
    void run(bool triggered) override {
        ...
        for (int i = 0; i < num_iterations; i++) {
            ...
            if (!memh) {
                std::cerr << "Rank " << rank << ": Memory handle is NULL in stress test" << std::endl;
                return;
            }
            if (memh_size == 0) {
                std::cerr << "Rank " << rank << ": Memory handle size is 0 in stress test" << std::endl;
                return;
            }
...
public:
    ...
    ucc_status_t set_input(int iter_persistent = 0) override {
        ...
        for (size_t i = 0; i < test_buffers.size(); i++) {
            memset(test_buffers[i], 0xDD + rank + iter_persistent, buffer_sizes[i]);
        }
        return UCC_OK;
    }
    ...
    void run(bool triggered) override {
        ...
        for (size_t i = 0; i < buffer_sizes.size(); i++) {
            ...
            if (!memh) {
                std::cerr << "Rank " << rank << ": Memory handle is NULL in multi-size test" << std::endl;
                return;
            }
            if (memh_size == 0) {
                std::cerr << "Rank " << rank << ": Memory handle size is 0 in multi-size test" << std::endl;
                return;
            }
